...
def searchActor(): # When searching for actor
    while True:
        query =input("Search for movies by actor: ")
        if len(query) == 0:
            print("Thank you!")
            break
        else:
...
def findResultsActor(query):
    titles= []
    for movie in data:
        if query in movie["Actors"]:
            titles.append(movie["Title"])
    return titles


def searchGenre(): # When searching for genre
    while True:
        query =input("Search for movies by genre: ")
        if len(query) == 0:
            print("Thank you!")
            break
        else:
...
def printResults(titles): # prints the results
    print("These movies matched your search: \n")
    for title in titles:
...
def main():
    ...
    searchmethod = input("Actor or genre?")
    if searchmethod == "actor":
        searchActor()
    else:
        searchGenre()
